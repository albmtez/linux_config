# Contents

[Linux configuration](./linux_system_config.md)

* [Remote server configuracion](./linux_system_config.md#Remote-server-configuracion)
* [Base configuration](./linux_system_config.md#Base-configuration)
* [Desktop configuration](./linux_system_config.md#Gnome-desktop-configuration)
* [Applications install](./linux_system_config.md#Applications-install)

[Develop environment](./develop_environment.md)

* [Basic tools](./develop_environment.md#Basic-tools)
* [Virtualization and provisioning](./develop_environment.md#Virtualization-and-provisioning)
* [Docker / Kubernetes](./develop_environment.md#Docker-/-Kubernetes)
* [IDE / Code editors](./develop_environment.md#IDE-/-Code-editors)
* [Languages / SDKs](./develop_environment.md#Languages-/-SDKs)
* [Services](./develop_environment.md#Services)
* [Desktop Applications](./develop_environment.md#Desktop-Applications)

[Vagrant base box creation](./vagrant_base_boxes.md)

* [Linux system configuration](./vagrant_base_boxes.md#Linux-system-configuration)
* [Create virtualbox boxed](./vagrant_base_boxes.md#Create-virtualbox-boxed)
* [Create libvirt boxed](./vagrant_base_boxes.md#Create-libvirt-boxed)

---

TODO: presentación